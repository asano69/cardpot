...
### 3. 順序保証がない（レース条件）（対応済み）

`applyRecords` がローカルの `updated` より古いレコードを書き込まないようにした。`writeChain` の直列化は JS 側の順序だけを保証するが、PocketBase 上の更新順序とは一致しないため、この比較で最終的な勝者を決める。

残る制約: 削除されたカードはローカルから物理的に消えるため、削除より後に届いた古い更新イベントを比較できず、カードが復活する可能性がある。実害は小さいと判断して墓標（tombstone）は導入していない。
